<template>
    <div class="flex items-center rounded bg-white mb-4">
        <input 
            ...
            @keydown.enter="$parent.go()"
            @keydown.up.prevent="$parent.move(-1)"
            @keydown.down.prevent="$parent.move(1)"
        >
        <div class="group rounded-r px-4 py-2 cursor-pointer" @click="$emit('toggle')">
